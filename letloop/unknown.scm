...
;; Use unicode NFKD normalization to avoid problems because of the input method
...
(define unknown-compute-x
  (lambda (salt identity password)
    (define ip (pk (argon2id
                    (unknown-bytevector salt)
                    (bytevector-append (unknown-bytevector identity) (unknown-bytevector  password)))))
    (let ((x (make-unknown 'x 32)))
      (define hasher (make-blake3))
      (blake3-update! hasher (unknown-bytevector salt))
      (blake3-update! hasher ip)
      (unknown-bytevector! x (blake3-finalize hasher 32))
      x)))

(define unknown-compute-verifier
  (lambda (parameter salt identity password)
    (values identity
            password
            salt
            (let ((verifier (make-unknown 'verifier 256)))
  ...
  (lambda (client B)
    ...
    (define S
      (unknown-client-compute-S
       ...
       B
       (unknown-compute-u (unknown-client-A client) B)))
...
(call-with-values (lambda () (unknown-compute-verifier PARAMETER-2048
                                                       (make-random-unknown 'salt 25)
                                                       (make-random-unknown 'identity 25)
                                                       (make-random-unknown 'password 25)))
  (lambda (identity password salt verifier)

    (define client
      (make-unknown-client PARAMETER-2048
                           (make-random-unknown 'client-secret 1536)
                           salt
                           identity
                           password))
    ...
    (define server
      (make-unknown-server PARAMETER-2048
                           (make-random-unknown 'server-secret 1536)
                           salt
                           identity
                           verifier))

    ;; (pk 'identity (unknown-bytevector identity))
    ;; (pk 'password (unknown-bytevector password))
    ;; (pk 'salt (unknown-bytevector salt))
    ...
    (pk (unknown-client-check-M2 client (unknown-server-M2 server)))
    (pk (unknown-server-check-M1 server (unknown-client-M1 client)))))
